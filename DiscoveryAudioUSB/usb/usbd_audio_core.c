...
static uint8_t usbd_audio_CfgDesc[AUDIO_CONFIG_DESC_SIZE] =
{
  ...
  0x02,                                 /* bNrChannels */
  HALF_WORD_BYTES,                      /* XXXX bSubFrameSize : 2 Bytes per frame (16bits) */
  SAMPLE_BITS,                          /* XXXX bBitResolution (16-bits per sample) */
  0x01,                                 /* bSamFreqType only one frequency supported */
  SAMPLE_FREQ(USBD_AUDIO_FREQ),         /* XXXX Audio sampling frequency coded on 3 bytes */
  /* 11 byte*/
  ...
  USB_ENDPOINT_TYPE_ASYNCHRONOUS,        /* USB_ENDPOINT_TYPE_ISOCHRONOUS: bmAttributes */
...
void usbd_ConfigureAudio(int mode)
{
	if (mode == 0)
	{
		usbd_audio_CfgDesc[87] = HALF_WORD_BYTES;
		usbd_audio_CfgDesc[88] = SAMPLE_BITS;
		usbd_audio_CfgDesc[90] = (uint8_t)USBD_AUDIO_FREQ;
		usbd_audio_CfgDesc[91] = (uint8_t)(USBD_AUDIO_FREQ >>  8);
		usbd_audio_CfgDesc[92] = (uint8_t)(USBD_AUDIO_FREQ >> 16);
		usbd_audio_CfgDesc[97] = (uint8_t)((( USBD_AUDIO_FREQ * 2 * HALF_WORD_BYTES)/1000) & 0xFF);
		usbd_audio_CfgDesc[98] = (uint8_t)((((USBD_AUDIO_FREQ * 2 * HALF_WORD_BYTES)/1000) >> 8) & 0xFF);
		sAUDIO_OUT_PACKET = AUDIO_OUT_PACKET;
		sUSBD_AUDIO_FREQ = USBD_AUDIO_FREQ;
	}
	else
	{
		usbd_audio_CfgDesc[87] = HALF_WORD_BYTES2;
		usbd_audio_CfgDesc[88] = SAMPLE_BITS2;
		usbd_audio_CfgDesc[90] = (uint8_t)USBD_AUDIO_FREQ2;
		usbd_audio_CfgDesc[91] = (uint8_t)(USBD_AUDIO_FREQ2 >>  8);
		usbd_audio_CfgDesc[92] = (uint8_t)(USBD_AUDIO_FREQ2 >> 16);
		usbd_audio_CfgDesc[97] = (uint8_t)((( USBD_AUDIO_FREQ2 * 2 * HALF_WORD_BYTES2)/1000) & 0xFF);
		usbd_audio_CfgDesc[98] = (uint8_t)((((USBD_AUDIO_FREQ2 * 2 * HALF_WORD_BYTES2)/1000) >> 8) & 0xFF);
		sAUDIO_OUT_PACKET = AUDIO_OUT_PACKET;
		sUSBD_AUDIO_FREQ = USBD_AUDIO_FREQ2;
	}
}
